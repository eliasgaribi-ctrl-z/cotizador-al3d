...
- **Cotizar con IA** — analiza un JPG o PDF del proyecto y propone las partidas. Si ya tienes trabajo capturado, lo **conserva** y agrega las de la IA al final; reemplazar sigue a un toque, pero ahora es una decisión que se toma a propósito y no por omisión. También puede analizar **la imagen que acabas de medir en el escalador**, con tus cotas: ahí las medidas ya no las estima, las toma tal cual. La imagen analizada se guarda con la cotización, así que sigue ahí si recargas o cierras la pestaña.
  ...
  **Cuando el proveedor está saturado, insiste la app y no tú.** Los planes gratuitos contestan «model is overloaded» o «rate limit» a cualquier hora, y eso salía como un error seco que obligaba a volver a pulsar *Analizar* hasta que sonara la flauta. Ahora reintenta sola —cuatro veces, con esperas crecientes— y si el proveedor sigue caído pasa al siguiente modelo y al siguiente proveedor que **tenga key guardada en este dispositivo**, diciendo en pantalla con quién está hablando; si acabó contestando otro, lo avisa, porque el borrador viene de otra IA y quien lo revisa tiene derecho a saberlo. Los errores que no se arreglan reintentando —key inválida, modelo inexistente— no gastan intentos: se saltan al siguiente. Guardar la key de **dos** proveedores es lo único que hay que hacer para que la saturación de uno deje de detener la cotización.
