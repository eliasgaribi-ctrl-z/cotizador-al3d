...
líneas con su CSS y su JavaScript en línea:
    ...
    css/sistema.css           EL sistema de diseño: tokens, estructura, las siete capas. Lo comparten las tres páginas
...
Las dos aparecieron cuando se escribieron estas dos:
    ...
    pruebas/navegador/total-que-viaja.mjs    el total vuela del paso 1 al 2, y no vuela si pediste menos movimiento
...
La del total existe por lo mismo que las otras: la transición de elemento compartido se rompe
en silencio de cuatro maneras y ninguna da error de consola. Colgada del sitio equivocado, solo
volaba por uno de los cuatro caminos que llevan al paso 2. Medida en coordenadas de la ventana
en vez del documento, el scroll que hace el cambio de pantalla se sumaba al desplazamiento y el
número salía volando desde fuera de la pantalla. Con la escala sacada de una caja estirada por
su contenedor, el número se encogía desde 1,8 veces su destino, o sea contando lo contrario de
lo que pasa. Y con el `transform` sin limpiar al terminar, el resumen fijo de una partida se
colaba por encima al hacer scroll. Las cuatro se ven midiendo; ninguna, mirando.
